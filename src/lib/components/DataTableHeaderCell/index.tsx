...
const DataTableHeaderCell: React.FC<Props> = ({ indexOfTheCell, contentOfTheCell, changeSortOrder, setSelectedColumnIndex, selectedColumnIndex, theme }): JSX.Element => {
    ...
    return (
        <th id={`${indexOfTheCell}`} className={`${selectedColumnIndex === indexOfTheCell ? "data-table-header-cell__selected" : "data-table-header-cell "}`} scope="col" key={`header-${indexOfTheCell}`} onClick={(e) => onSortByHeader(e)} style={{display: ""}}>
            <div className="data-table-header-cell-content">
                ...
                {selectedColumnIndex === indexOfTheCell ?
                    (
                        <svg viewBox="0 0 24 24" width="12" height="12" fill={`${theme === "dark" ? "#bababa" : "#000000"}`}>
